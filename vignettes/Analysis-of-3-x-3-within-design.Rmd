...
# Analysis of a 3 (between) x 3 (wthin) analysis design
...
We don't actually want the non-interacting contrasts in the above mentioned table, so lets remove them now.

```{r}

interaction_fx = summary(interaction_fx)
interaction_fx = interaction_fx %>% filter(!(Group_custom == "T1*" & Time_custom == "Post*"),
                                           !(Group_custom == "T2*" & Time_custom == "Post*"),
                                           !(Group_custom == "Ctrl*" & Time_custom == "Post*"),
                                           !(Group_custom == "T1*" & Time_custom == "FU*"),
                                           !(Group_custom == "T2*" & Time_custom == "FU*"),
                                           !(Group_custom == "Ctrl*" & Time_custom == "FU*"))
```

We also want to estimate the main effects of each set of between and within contrasts,
and combine the contrast tables together. For each of the between and within, I am going to compute
the appropriate contrasts, and then join the tables together.
...
all_fx = rbind(interaction_fx, summary(between_fx), summary(within_fx))
...
this_alpha = 3*0.05
psyci(mod, all_fx, method="bf", alpha=this_alpha)
```


As this method is planned, we use 
